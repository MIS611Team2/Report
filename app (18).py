...
def calculate_indicator_scores(df):
    """
    Calculate real 0–10 indicator scores using a per-respondent approach.
    For each indicator, groups all mapped columns and checks whether AT LEAST ONE
    column per row signals a positive response — this avoids inflated scores from
    duplicate child-response columns (.1, .2 suffixes).
    """
    n = len(df)
    if n == 0:
        return {}
    ...
    # Group all columns by indicator
    ind_col_groups = {}   # ind_name → {rule → [col, ...]}
    for col in df.columns:
        norm = _normalise_col_key(col)
        entry = mapping_lookup.get(norm)
        if entry is None:
        ...
        ind_name = entry[1]
        rule     = entry[2]
        ind_col_groups.setdefault(ind_name, {}).setdefault(rule, []).append(col)

    scores = {}
    for ind_name, rule_cols in ind_col_groups.items():
        vals = []
        for rule, cols in rule_cols.items():
            try:
                if rule == 'positive':
                    # % of respondents where ≥1 child showed this positive emotion
                    val = df[cols].notna().any(axis=1).mean()
                    vals.append(float(val))

                elif rule == 'negative':
                    # Fewer negative selections = better; invert
                    val = 1.0 - df[cols].notna().any(axis=1).mean()
                    vals.append(float(val))

                elif rule == 'binary':
                    # % of respondents where ≥1 child had this behaviour
                    val = df[cols].notna().any(axis=1).mean()
                    vals.append(float(val))

                elif rule == 'binary_inverse':
                    # "Has attended before?" — No = first time = good for access
                    no_count = df[cols].apply(
                        lambda s: s.astype(str).str.lower().str.strip().eq('no')
                    ).any(axis=1).mean()
                    vals.append(float(no_count))

                elif rule in ('stars', 'nps', 'continuous'):
                    # Average across all matching columns, normalise to 0–1
                    combined = pd.concat([pd.to_numeric(df[c], errors='coerce') for c in cols])
                    valid = combined.dropna()
                    if not valid.empty and valid.max() > 0:
                        vals.append(float(valid.mean() / valid.max()))
