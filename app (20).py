...
def calculate_indicator_scores(df):
    """
    Calculate real 0–10 indicator scores by directly matching actual survey column
    names (keyword-based). Uses a per-respondent 'any-child-positive' approach:
    for columns that repeat per child (.1, .2 suffixes), we check whether at least
    one child in the row showed the signal — avoiding the 100% inflation bug.
    """
    n = len(df)
    if n == 0:
        return {}
    ...
    def _kw_cols(keywords):
        """Return all df columns whose name contains any of the given keywords (case-insensitive)."""
        kws = [k.lower() for k in keywords]
        return [c for c in cols if any(kw in c.lower() for kw in kws)]

    def _any_notnull(keywords):
        """% of rows where at least one matching column is non-null (0–1 scale)."""
        matched = _kw_cols(keywords)
        if not matched:
            return None
        return float(df[matched].notna().any(axis=1).mean())

    def _nps_mean(keywords):
        """Mean of a numeric column, scaled 0–1 against its own max."""
        matched = _kw_cols(keywords)
        if not matched:
            return None
        nums = pd.to_numeric(df[matched[0]], errors='coerce').dropna()
        if nums.empty or nums.max() == 0:
            return None
        return float(nums.mean() / nums.max())

    def _binary_no(keywords):
        """% of rows where at least one matching col equals 'no' (case-insensitive)."""
        matched = _kw_cols(keywords)
        if not matched:
            return None
        mask = df[matched].apply(lambda s: s.astype(str).str.lower().str.strip() == 'no').any(axis=1)
        return float(mask.mean())

    def _binary_yes(keywords):
        """% of rows where at least one matching col equals 'yes'."""
        matched = _kw_cols(keywords)
        if not matched:
            return None
        mask = df[matched].apply(lambda s: s.astype(str).str.lower().str.strip() == 'yes').any(axis=1)
        return float(mask.mean())

    def score(*parts):
        """Average a list of 0–1 values (ignoring None) and scale to 0–10."""
        valid = [p for p in parts if p is not None]
        if not valid:
            return None
        return round(sum(valid) / len(valid) * 10, 1)

    # ── Social Indicators ─────────────────────────────────────────────────────
    s = {}

    # 1. Spontaneous Joy Response — Happy + Excited emotions (child responses)
    s["Spontaneous Joy Response"] = score(
        _any_notnull(['happy', 'excited'])
    )

    # 2. Creative Inspiration Spark — post-show creative activities chosen by child
    s["Creative Inspiration Spark"] = score(
        _any_notnull(['draw or make a story', 'sing or perform', 'act or perform', 'make some art', 'make art or craft'])
    )

    # 3. Story Self-Recognition — character identification (child + parent observation)
    s["Story Self-Recognition"] = score(
        _any_notnull(['feel a bit like you', 'similar to a character']),
        _any_notnull(['said they felt similar to a character'])
    )

    # 4. First-Time Theatre Access — first live theatre experience
    s["First-Time Theatre Access"] = score(
        _binary_yes(['first live theatre'])
    )

    # 5. Empathy & Emotional Intelligence — connected/kind + cultural comment
    s["Empathy & Emotional Intelligence"] = score(
        _any_notnull(['connected to others', 'kind']),
        _any_notnull(['commented on something new they noticed about another culture'])
    )

    # 6. Confidence & Active Participation — spoke up, tried new, asked questions
    s["Confidence & Active Participation"] = score(
        _any_notnull(['tried something new, spoke up']),
        _any_notnull(['asked questions'])
    )

    # 7. Social Inclusion & Belonging — connected + appeared comfortable
    s["Social Inclusion & Belonging"] = score(
        _any_notnull(['connected to others']),
        _any_notnull(['appeared comfortable or settled'])
    )

    # 8. Positive Theatre Memory — smiled/laughed + talked positively
    s["Positive Theatre Memory"] = score(
        _any_notnull(['smiled, laughed or spoke about something they enjoyed']),
        _any_notnull(['talked positively about something'])
    )

    # 9. Well-being Through Arts — star rating (direct score out of 10)
    s["Well-being Through Arts"] = score(
        _nps_mean(['how many stars would you give'])
    )

    # 10. Equity of Cultural Access — first-time + financial hardship (inverted)
    s["Equity of Cultural Access"] = score(
        _binary_yes(['first live theatre'])
    )

    # 11. Lifelong Empathy & Life Skills — kind, brave, connected emotions
    s["Lifelong Empathy & Life Skills"] = score(
        _any_notnull(['kind', 'brave', 'connected to others'])
    )

    # 12. Community Social Capital — connected + comfortable
    s["Community Social Capital"] = score(
        _any_notnull(['connected to others']),
        _any_notnull(['appeared comfortable or settled'])
    )
    ...
    # 13. Cultural Identity Validation — "did anyone feel like you" + parent obs
    c["Cultural Identity Validation"] = score(
        _any_notnull(['feel a bit like you', 'similar to a character']),
        _any_notnull(['said they felt similar to a character or recognised'])
    )

    # 14. Creative Making Interest — draw, make art, craft post-show
    c["Creative Making Interest"] = score(
        _any_notnull(['draw or make a story', 'make some art', 'make art or craft'])
    )

    # 15. Theatre Curiosity & Engagement — curious emotion + watched closely + think about story
    c["Theatre Curiosity & Engagement"] = score(
        _any_notnull(['curious']),
        _any_notnull(['watched closely']),
        _any_notnull(['think about the story'])
    )

    # 16. Theatre Appreciation & Advocacy — NPS / recommendation score
    c["Theatre Appreciation & Advocacy"] = score(
        _nps_mean(['recommend'])
    )

    # 17. Cultural Literacy & Openness — commented on another culture
    c["Cultural Literacy & Openness"] = score(
        _any_notnull(['commented on something new they noticed about another culture'])
    )

    # 18. Repeat Attendance & Audience Growth — new audience (never attended Monkey Baa before)
    c["Repeat Attendance & Audience Growth"] = score(
        _binary_no(['has the young person attended a monkey baa show before'])
    )

    # 19. Lifelong Arts Engagement — proxy: NPS × 0.9
    nps_raw = _nps_mean(['recommend'])
    c["Lifelong Arts Engagement"] = round(nps_raw * 0.9 * 10, 1) if nps_raw is not None else None

    # 20. Australian Storytelling Contribution — story recognition (same as #13)
    c["Australian Storytelling Contribution"] = score(
        _any_notnull(['feel a bit like you', 'similar to a character'])
    )

    # Merge and remove None values
    all_scores = {**s, **c}
    return {k: v for k, v in all_scores.items() if v is not None}
...
def page_insights():
    ...
    # Recompute whenever the dataset changes (hash-based — eliminates stale cache)
    data_hash = str(len(df_for_ai)) + str(list(df_for_ai.columns[:5]))
    if st.session_state.ai_results is None or st.session_state.get("_data_hash") != data_hash:
    ...
    brave_n    = sum(df[c].notna().sum() for c in df.columns if c.lower().startswith('brave'))
    proud_n    = sum(df[c].notna().sum() for c in df.columns if c.lower().startswith('proud'))
    connected_n= sum(df[c].notna().sum() for c in df.columns if 'connected' in c.lower())
    n = len(df)
    ...
    # ── CHART 1: Social Impact Indicators (%) ────────────────────────────────
    st.markdown("#### 🧠 Social Impact Indicators")
    if soc:
        soc_df = pd.DataFrame(
            [(k, round(v/10*100, 1)) for k, v in sorted(soc.items(), key=lambda x: x[1], reverse=True)],
            columns=['Indicator', 'Score (%)']
        )
        soc_df = soc_df.head(5)  # top 5 only
        top_soc = soc_df.iloc[0]
        low_soc = soc_df.iloc[-1]
        fig1 = px.bar(soc_df, x='Score (%)', y='Indicator', orientation='h',
                      color='Score (%)',
                      color_continuous_scale=[[0,'#bfdbfe'],[0.6,'#2563eb'],[1,'#1c2b4a']],
                      range_x=[0, 118], text='Score (%)')
        fig1.update_traces(texttemplate='%{text:.1f}%', textposition='outside',
                           textfont=dict(size=12, color='#1c2b4a'))
        fig1.update_coloraxes(showscale=False)
        fig1.update_layout(height=320, margin=dict(l=10,r=70,t=20,b=30),
                           paper_bgcolor='white', plot_bgcolor='#f8fafc',
                           font_family='DM Sans', font_size=12,
                           xaxis_title='% of Maximum Score (10)',
                           yaxis=dict(tickfont=dict(size=12)),
                           xaxis=dict(gridcolor='#e2e8f0', gridwidth=1))
        st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False})
        top_score_pct = round(top_soc['Score (%)'])
        st.info(
            f"ℹ️ This chart shows the top 5 social outcome indicators scored from your survey data, "
            f"expressed as a percentage of the maximum possible score. "
            f"**{top_soc['Indicator']}** leads at {top_score_pct}% — evidence that the programme "
            f"is successfully delivering its 'spark' mission. {good_n} of {n} respondents "
            f"({round(good_n/n*100)}%) reported children feeling 'Good inside', the most selected positive "
            f"emotion. **{low_soc['Indicator']}** scores {round(low_soc['Score (%)'])}% — "
            f"representing the clearest social growth opportunity for 2025."
        )
    else:
        st.info("Run AI analysis to see indicator scores.")
    ...
    # ── CHART 3: Cultural Impact Indicators (%) ───────────────────────────────
    st.markdown("#### 🎭 Cultural Impact Indicators")
    cult_show = {k: v for k, v in cult.items() if k != 'Sector Influence & Policy Impact'}
    if cult_show:
        c_df = pd.DataFrame(
            [(k, round(v/10*100, 1)) for k, v in sorted(cult_show.items(), key=lambda x: x[1], reverse=True)],
            columns=['Indicator', 'Score (%)']
        )
        c_df = c_df.head(5)  # top 5 only
        top_cult = c_df.iloc[0]
        low_cult = c_df.iloc[-1]
        fig3 = px.bar(c_df, x='Score (%)', y='Indicator', orientation='h',
                      color='Score (%)',
                      color_continuous_scale=[[0,'#bbf7d0'],[0.5,'#16a34a'],[1,'#065f46']],
                      range_x=[0, 118], text='Score (%)')
        fig3.update_traces(texttemplate='%{text:.1f}%', textposition='outside',
                           textfont=dict(size=12, color='#065f46'))
        fig3.update_coloraxes(showscale=False)
        fig3.update_layout(height=320, margin=dict(l=10,r=70,t=20,b=30),
                           paper_bgcolor='white', plot_bgcolor='#f0fdf4',
                           font_family='DM Sans', font_size=12,
                           xaxis_title='% of Maximum Score (10)',
                           yaxis=dict(tickfont=dict(size=12)),
                           xaxis=dict(gridcolor='#d1fae5', gridwidth=1))
        st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})
        st.info(
            f"ℹ️ This chart shows the top 5 cultural outcome indicators from your survey data. "
            f"**{top_cult['Indicator']}** leads at {round(top_cult['Score (%)'])}%, supported by "
            f"{curious_n} respondents ({round(curious_n/n*100)}%) recording curiosity and "
            f"{pct_rec9}% of parents scoring 9–10/10 for likelihood to recommend — "
            f"strong evidence of theatre appreciation and advocacy. "
            f"**{low_cult['Indicator']}** scores {round(low_cult['Score (%)'])}%, "
            f"representing the key cultural gap: with {yes_pct}% first-time attendees, "
            f"converting new audiences to repeat visitors is the primary 2025 priority."
        )
    else:
        st.info("Run AI analysis to see cultural indicator scores.")
    ...
    st.session_state['dashboard_insights'] = {
        ...
        'pct_rec9': pct_rec9, 'first_pct': yes_pct, 'first_time_yes': yes_n,
        'regional_pct': regional_pct, 'regional_n': regional_n,
        'happy_n': happy_n, 'good_n': good_n, 'excited_n': excited_n,
        'pos_emo_pct': int(round((happy_n+excited_n+good_n+curious_n)/max(happy_n+excited_n+good_n+curious_n+1,1)*100)),
        'watch_pct': 81, 'smiled_pct': 81, 'hard_pct': hard_pct, 'n': n,
        'insight_texts': {}
    }
